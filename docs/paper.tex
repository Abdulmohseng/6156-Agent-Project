...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{neatagent_images/plan-and-act.png}
    \caption{Overview of the Plan-and-Act framework, illustrating the separation between high-level planning and step-by-step execution}
...
For evaluation, I created a dataset of 10 files with generic names (file1.txt, file4.pdf, etc.) covering different content types such as invoices, health reports, gym logs, photos, and CSVs. The goal was to see whether the agent could classify files correctly based on \textit{content} alone, since the filenames give no signal. I compared three models of different sizes: \texttt{qwen2.5-coder:14b}, \texttt{qwen3:8b}, and \texttt{llama3.2:3b}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{NeatAgent_architecture.png}
    \caption{PLAN-AND-ACT architecture}
    \label{fig:architecture}
\end{figure}
...
\subsection{RQ3: How do local models compare to each other and to a frontier model?}

To get a more complete picture, I also evaluated Claude Code (claude-sonnet-4-6, Anthropic's frontier model) on the same dataset. Claude Code operated directly on the files rather than through the NeatAgent pipeline, so I built a separate folder-based evaluator (\texttt{eval/eval\_folder.py}) that scores any organized folder against the ground truth without requiring a manifest. Claude Code also renamed every file to a descriptive name, so I recorded the rename mapping and passed it to the evaluator.

\begin{table}[H]
\centering
\caption{\texttt{model\_test} results — NeatAgent (three local models) vs.\ Claude Code}
\begin{tabular}{lrrrrr}
\toprule
\textbf{System} & \textbf{Model} & \textbf{Coverage} & \textbf{Placement Acc.} & \textbf{Renamed files} & \textbf{Time} \\
\midrule
NeatAgent & \texttt{qwen2.5-coder:14b} & 100\% & 60\% & yes & $\sim$90s \\
NeatAgent & \texttt{qwen3:8b}          & 100\% & 50\% & yes & $\sim$70s \\
NeatAgent & \texttt{llama3.2:3b}       & 100\% & 40\% & yes & $\sim$40s \\
Claude Code & claude-sonnet-4-6        & 100\% & \textbf{80\%} & yes & interactive \\
\bottomrule
\end{tabular}
\end{table}

Claude Code achieved 80\% placement accuracy — the highest of all systems tested — and also produced the most descriptive renames (e.g., \texttt{file4.pdf} became \texttt{cloud\_services\_invoice\_2026\_0412.pdf}). The two misses were \texttt{file3.txt} (gym log) and \texttt{file5.pdf} (health report), both placed in \texttt{personal/} rather than a dedicated \texttt{health/} folder. Claude Code chose to merge health and personal into one broader category, which is a reasonable organizational choice even if it doesn't match the ground truth exactly.

All four systems achieved 100\% coverage. The gap between the best local model (60\%) and Claude Code (80\%) is meaningful but not as large as one might expect. NeatAgent with \texttt{qwen2.5-coder:14b} closes about two-thirds of the gap while running entirely offline and for free.
...
The most lasting thing I took from this project is that output format is more important than model capability for agentic tasks. I spent time trying to fix the hallucination problem through better prompting with more explicit instructions, stronger warnings, different phrasing. None of it helped. Changing the structure of what the model was asked to output fixed it immediately. When you can constrain the output schema so that the error can't be represented, you've solved the problem in a way that's reliable regardless of which model you're using.
